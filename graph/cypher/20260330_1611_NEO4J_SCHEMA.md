...
The graph models two social situations in which a person gives feedback on a painting. The core insight is that the **literal meaning** of an utterance (always "praise" in this toy example) can diverge from the **intended meaning** (sarcasm vs. genuine praise) depending on paralinguistic cues — tone and facial expression — in a shared context.

**Key structural ideas:**

| Concept | Role |
|---|---|
| `Person` | The speaker in a situation |
| `Statement` | A per-utterance instance node carrying the raw text |
| `Tone`, `Expression`, `Context` | Observable cue vocabulary nodes |
| `LiteralMeaning` | Surface meaning of the words (shared singleton `"praise"`) |
| `IntendedMeaning` | Social interpretation (`sarcasm`, `praise`, etc.) |
| `Signal` | Derived token node capturing that a cue value was observed; bridges vocabulary to pattern matching |
| `Pattern` | Reusable conjunctive rule: REQUIRES a set of Signals, PREDICTS an IntendedMeaning with a probability |
| `Situation` | One concrete observed episode; links to all its cues and its derived Signal set |

**Two situations are modelled:**
- **Situation A** (`painting_feedback_eye_roll`): Henry says "Congratulations, it's definitely original" with a dry tone and an eye roll → fires the **Sarcasm** pattern (probability 0.8).
- **Situation B** (`painting_feedback_awe`): Alice says "It's great, it's very original" with an enthusiastic tone and awe → fires the **Genuine Praise** pattern (probability 0.85).
...
| `Pattern` | `patternId` | `description`, `match_mode`, `min_coverage` |
...
| `REQUIRES` | Pattern → Signal | `weight` (float, default 1.0) |
| `PREDICTS` | Pattern → IntendedMeaning | `probability` (float) |

### Signal identity convention
Each `Signal` node is a **shared vocabulary token** identified by `(kind, valueId)`. Both situations share the same `sig_lm_praise` signal node (since both statements have literal meaning "praise"). Situation-specific attachment is expressed via `Situation -[:HAS_SIGNAL]-> Signal`.
